...
shared void createScheduler (
	...
	if (exists messageSource) {
		...
		if (exists messageSourceConfig) {
			request.put(Chime.key.messageSourceOptions, messageSourceConfig);
		}
	}
	if (exists eventProducer) {
		request.put(Chime.key.eventProducer, eventProducer);
	}
	if (exists eventProducerOptions) {
		request.put(Chime.key.eventProducerOptions, eventProducerOptions);
	}
